...
//swing
u8 flag_swing = 0;
int16_t swing_dir = 1;   //dir = 1 : +    dir = -1 : -
...
void move(int16_t speedX, int16_t speedY, int16_t rad)
{
		...
		speedA = CMSpeedLegalize(speedA,5000);
		speedB = CMSpeedLegalize(speedB,5000);
		speedC = CMSpeedLegalize(speedC,5000);
		speedD = CMSpeedLegalize(speedD,5000);
	...
	switch(keyboardvalue & (KEY_PRESSED_OFFSET_W|KEY_PRESSED_OFFSET_S))
		{   
			...
			default:      //没有输入时
				...
				if(m>1)
				{
					if(keymove_y>0){keymove_y=keymove_y-1*step;}
					else if(keymove_y<0){keymove_y=keymove_y+1*step;}
				}
				break;	
	  }
