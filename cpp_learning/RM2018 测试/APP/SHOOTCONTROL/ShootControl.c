...
void GunShootAction(void)						
{
	...
	if(t1_5ms == 0 && gun_count > 0 && flag_trigger == 1)	  //t1_5ms 为0时，摇杆s1处于1位置或鼠标左键按下，可以射击子弹
	 {
...
{
	if(extShootData.bulletFreq >= 10)
	{
	if(extGameRobotState.robotLevel == 1 && extPowerHeatData.shooterHeat0 >= 1600 - extShootData.bulletSpeed^2)
		flag_trigger = 0;
	else if(extGameRobotState.robotLevel == 2 && extPowerHeatData.shooterHeat0 >= 3000 - extShootData.bulletSpeed^2)
		flag_trigger = 0;
	else if(extGameRobotState.robotLevel == 3 && extPowerHeatData.shooterHeat0 >= 6000 - extShootData.bulletSpeed^2)
		flag_trigger = 0;
	else 
		flag_trigger = 1;
	}
	...
	{
	if(extGameRobotState.robotLevel == 1 && extPowerHeatData.shooterHeat0 >= 1600)
		flag_trigger = 0;
	if(extGameRobotState.robotLevel == 2 && extPowerHeatData.shooterHeat0 >= 3000)
		flag_trigger = 0;
	if(extGameRobotState.robotLevel == 3 && extPowerHeatData.shooterHeat0 >= 6000)
		flag_trigger = 0;
	else 
		flag_trigger = 1;
	}
