...
void GimbalPidInit(void)
{
	...
	PID_Init(&MotorPID_view.View,		PIDVAL_VIEW_p,PIDVAL_VIEW_i, PIDVAL_VIEW_d,-60,60,0);//13 0 0
	SetPIDCR(&MotorPID_view.View,(1<<PEN)+(1<<IEN)+(1<<DEN)+(1<<PIDEN)+(1<<INTERVAL));
}
...
void GimbalControlValSet(void)	
{
	...
	else if(remoteState == NORMAL_REMOTE_STATE )     //Use remote 
	{
			...
			Gimbal_control.angle_pitch_set += 0.0002f	*	(RC_Ex_Ctl.rc.ch3);      //Pitch轴转角与摇杆值呈线性比例关系
			
			Gimbal_control.angle_yaw_current = -(Yaw   - Yaw_Offset  ) *57.3f;        //获得当前Yaw轴角度，用来控制Yaw轴位置环
			#ifdef PITCH_BORDER                                                       //横版
			Gimbal_control.angle_pitch_current = (Pitch  - Pitch_Offset) *57.3f;      //云台的PITCH轴对应陀螺仪Pitch轴，获得当前Pitch轴角度，仅在GimbalControl_PitchAngel()中使用
			#endif
			  
			#ifdef ROLL_BORDER                                                        //竖版
			Gimbal_control.angle_pitch_current = -(Roll  - Roll_Offset) *57.3f;      //云台的PITCH轴对应陀螺仪Pitch轴，获得当前Pitch轴角度，仅在GimbalControl_PitchAngel()中使用
			#endif
	...
	{
	    Gimbal_control.angle_pitch_set 					-= 0.1*MOUSE_PIT_SPEED * RC_Ex_Ctl.mouse.y;
	    Gimbal_control.angle_yaw_set	          += (abs(RC_Ex_Ctl.mouse.x)>40 ? (RC_Ex_Ctl.mouse.x>0 ? 40: -40 ) : RC_Ex_Ctl.mouse.x) * MOUSE_YAW_SPEED;
...
void GimbalMove(void)
{
	...
	else if(remoteState == KEY_REMOTE_STATE ) //键盘模式
	{
...
void GimbalControl_PitchAngel(float SetPosition)
{
	...
	#ifdef PITCH_BORDER //横版
	float NowSpeed = -mpu6500_real_data.Gyro_Y * 57.3f;					        //pitch轴当前角速度值
	#endif
